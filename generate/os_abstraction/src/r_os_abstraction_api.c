...
#define FREERTOS_HEAP_START_PRV_        ((void *)&Image$$FREERTOS_HEAP$$ZI$$Base)
#define FREERTOS_HEAP_END_PRV_          ((void *)&Image$$FREERTOS_HEAP$$ZI$$Limit)

extern uint32_t Image$$FREERTOS_HEAP$$ZI$$Base;
extern uint32_t Image$$FREERTOS_HEAP$$ZI$$Limit;

/* Note ucHeap is used in freeRTOS for fixed location fixed sized memory pool */
uint8_t ucHeap[configTOTAL_HEAP_SIZE] __attribute__ ((section(".bss.freertos_heap")));
...
#pragma section="FREERTOS_HEAP"

uint8_t ucHeap[configTOTAL_HEAP_SIZE] @ "FREERTOS_HEAP";
...
#else
#error "syscalls.c not implemented in os_abstraction module for your toolchain"
#endif

extern uint32_t ulPortInterruptNesting;
UBaseType_t uxSavedInterruptStatus;
static uint32_t s_interrupt_flag = 0;
...
void R_OS_InitMemManager (void)
{
    ...
    {
#if defined( __CC_ARM )                                      // CC_ARM toolchain
...
#elif defined( __ICCARM__ )                                     // IAR toolchain
        xHeapRegions[0].pucStartAddress = (uint8_t *)__section_begin("FREERTOS_HEAP");
...
void R_OS_EnterCritical (void)
{
    ...
    {
        s_interrupt_flag = taskENTER_CRITICAL_FROM_ISR();
    }
}
/**********************************************************************************************************************
...
void R_OS_ExitCritical (void)
{
    ...
    {
        taskEXIT_CRITICAL_FROM_ISR(s_interrupt_flag);
    }
}
/**********************************************************************************************************************
